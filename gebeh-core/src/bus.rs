use crate::{
    ...
    mbc::Mbc,
    ppu::{LcdControl, LcdStatus, Vram},
    serial::SerialControl,
};

pub fn external_bus_read<M: Mbc + ?Sized>(index: u16, mbc: &M, vram: &Vram, wram: &Wram) -> u8 {
    match index {
...
pub fn write<M: Mbc + ?Sized>(
    ...
    match index {
        ...
        INTERRUPT_ENABLE => cpu.interrupt_enable = Interrupts::from_bits_retain(value),
    }
}
