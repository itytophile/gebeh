...
enum DebugMode {
    None,
    Scanline,
    Pixel(u8), // previous scanline len
}
...
fn main() {
    ...
    let (_, mut mbc) = get_mbc(rom.as_slice(), InstantRtc::default()).unwrap();
    ...
    event_loop
        .run(|event, elwt| match event {
            ...
            Event::WindowEvent {
                ...
                let mut joypad = *emulator.get_joypad();
                match keycode {
...
fn drive_emulator(
    ...
    while start.elapsed() <= Duration::from_millis(33) {
        ...
        if *debug_mode == DebugMode::Scanline
            && let PpuStep::Drawing { dots_count: 4, .. } = &emulator.get_ppu().step
        {
            log::info!(
                "LY = {:03}, SCX = {:03}, SCY = {:03}",
                emulator.get_ppu().get_ly(),
                emulator.get_ppu().get_scx(),
                emulator.get_ppu().get_scy(),
            );
        }
        ...
        let scanline = match debug_mode {
            DebugMode::Pixel(previous_len) => {
                ...
                *previous_len = renderer.scanline.len();
                renderer.scanline.get_scanline()
            }
            _ => {
        ...
        let base = usize::from(emulator.get_ppu().get_ly()) * usize::from(WIDTH);
        for (pixel, color) in pixels[base..].iter_mut().zip(scanline.iter_colors()) {
            *pixel = color.into();
        }
        if emulator.get_ppu().get_ly() == HEIGHT - 1 || *debug_mode != DebugMode::None {
            break;
        }
    }
}
