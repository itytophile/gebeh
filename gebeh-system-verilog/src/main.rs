...
use crate::{
    ...
    drlatch_ee::{CanonicalDrlatchEe, DrlatchEe, canonicalize_drlatch_ee, parse_drlatch_ee},
    ...
    not::{Not, parse_not},
    tffnl::{CanonicalTffnl, Tffnl, canonicalize_tffnl, parse_tffnl},
};
...
fn main() {
    ...
    let canonical_instances: Vec<_> = instances
        ...
        .filter_map(|instance| match instance {
            ...
            Instance::DrlatchEe(drlatch_ee) => Some(CanonicalInstance::DrlatchEe(
                canonicalize_drlatch_ee(drlatch_ee, &nots_by_output),
            )),
            Instance::Tffnl(tffnl) => Some(CanonicalInstance::Tffnl(canonicalize_tffnl(
                tffnl,
                &nots_by_output,
            ))),
        })
...
impl<'a> CanonicalInstance<'a> {
    fn get_inputs(&self) -> Inputs<'_> {
        match self {
            ...
            CanonicalInstance::DrlatchEe(drlatch_ee) => {
                [drlatch_ee.ena.name, drlatch_ee.d.name, drlatch_ee.r_n.name]
                    .into_iter()
                    .collect()
            }
